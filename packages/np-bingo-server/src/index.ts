...
import { Ball, Card, Results, Winner } from './client-types';
...
io.on('connection', (socket: Socket) => {
   ...
   * @param player IPlayer
   */
  socket.on('checking-card', (room: Room, player: IPlayer) => {
  ...
  socket.on(
    'winning-card',
    (card: Card, player: IPlayer, methods: string[], data: Results) => {
      io.to(player.socket).emit('winner', {
        player: player,
        card: card,
        methods: methods,
        data: data,
